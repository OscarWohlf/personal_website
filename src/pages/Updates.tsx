...
function UpdateLinkButton({ link, lang }: { link: UpdateLink; lang: "en" | "nb" }) {
  ...
  const className = "px-3 py-1.5 text-xs rounded-xl border dark:border-zinc-700 hover:shadow"
      ...
      <ol className="mt-8 max-w-3xl space-y-8">
        {allUpdates.map((update) => (
          <li
            key={update.slug}
            id={update.slug}
            className="border-l pl-5 dark:border-zinc-800 scroll-mt-24"
          >
            ...
            {update.body?.[lang]?.map((paragraph) => (
              <p key={paragraph} className="mt-3 leading-relaxed text-zinc-700 dark:text-zinc-300">
                {paragraph}
              </p>
            ))}
            ...
            {update.tags?.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {update.tags.map((tag) => (
                  <span key={tag} className="text-xs px-2 py-1 rounded-full border dark:border-zinc-700">
                    {tag}
                  </span>
                ))}
              </div>
